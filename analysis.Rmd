...
```
## Parent-Child Clumping Difference
...
# pbmax <- length(wsizes) * length(zooms)
# pbidx <- 0
# pb <- txtProgressBar(max = pbmax, style = 3)
...
#   
#   r <- rast[[zoom]]
#   st_std <- GetisOrdStandardStats(r)
#   st_foc <- GetisOrdLocalStats(r, weight_matrix_circular(51))
#   
#   std_up <- eval_getisord(r, st_std, wsizes, wgen = wgen)
#   foc_up <- eval_getisord(r, st_foc, wsizes, wgen = wgen)
#   
#   results <- rbind(results, cbind(zoom, wsizes[-1], std_up[,2], foc_up[,2]))
  
  # plot(res_std, type = "n", ylim = c(0,1))
  # lines(res_std, col = "red")
  # lines(res_foc, col = "blue")
...
zoom_soh %>% dplyr::select(Zoom, "G*" = UpZoomGstar) -> y1
zoom_soh %>% dplyr::select(Zoom, "Focal G*" = UpFocalGstar) -> y2
...
zoom_soh %>% dplyr::select(Zoom, "G*" = DownZoomGstar) -> y1
zoom_soh %>% dplyr::select(Zoom, "Focal G*" = DownZoomFocalGstar) -> y2
...
mean <- aggregate(d3$Up, by = list(d3$W), mean)
sd <- aggregate(d3$Up, by = list(d3$W), sd)
meanG <- aggregate(d3global$Up, by = list(d3global$W), mean)
sdG <- aggregate(d3global$Up, by = list(d3global$W), sd)
plot(NA, ylab="Up", xlim=c(0,40), ylim=c(0,1), xlab=sprintf("Weight"))
plotCI(x=mean$Group.1, y=mean$x, uiw=sd$x, err="y", pch=20, slty=3, scol = "red", add=TRUE)
plotCI(x=meanG$Group.1, y=meanG$x, uiw=sdG$x, err="y", pch=20, slty=3, scol = "blue", add=TRUE)
lines(mean$Group.1, mean$x, col="red", lw=4)
lines(meanG$Group.1, meanG$x, col="blue", lw=4)
legend("topleft", c("Focal","Global") , fill=c("red","blue"))

mean <- aggregate(d3$Down, by = list(d3$W), mean)
sd <- aggregate(d3$Down, by = list(d3$W), sd)
meanG <- aggregate(d3global$Down, by = list(d3global$W), mean)
sdG <- aggregate(d3global$Down, by = list(d3global$W), sd)
plot(NA, ylab="Down", xlim=c(0,40), ylim=c(0,1), xlab=sprintf("Weight"))
plotCI(x=mean$Group.1, y=mean$x, uiw=sd$x, err="y", pch=20, slty=3, scol = "red", add=TRUE)
plotCI(x=meanG$Group.1, y=meanG$x, uiw=sdG$x, err="y", pch=20, slty=3, scol = "blue", add=TRUE)
lines(mean$Group.1, mean$x, col="red", lw=4)
lines(meanG$Group.1, meanG$x, col="blue", lw=4)
...
mean <- aggregate(d3$Up, by = list(d3$Z), mean)
sd <- aggregate(d3$Up, by = list(d3$Z), sd)
meanG <- aggregate(d3global$Up, by = list(d3global$Z), mean)
sdG <- aggregate(d3global$Up, by = list(d3global$Z), sd)
plot(NA, ylab="Up", xlim=c(1,5), ylim=c(0,1), xlab=sprintf("Aggregation"))
plotCI(x=mean$Group.1, y=mean$x, uiw=sd$x, err="y", pch=20, slty=3, scol = "red", add=TRUE)
plotCI(x=meanG$Group.1, y=meanG$x, uiw=sdG$x, err="y", pch=20, slty=3, scol = "blue", add=TRUE)
lines(mean$Group.1, mean$x, col="red", lw=4)
lines(meanG$Group.1, meanG$x, col="blue", lw=4)
...
mean <- aggregate(d3$Down, by = list(d3$Z), mean)
sd <- aggregate(d3$Down, by = list(d3$Z), sd)
meanG <- aggregate(d3global$Down, by = list(d3global$Z), mean)
sdG <- aggregate(d3global$Down, by = list(d3global$Z), sd)
plot(NA, ylab="Down", xlim=c(1,5), ylim=c(0,1), xlab=sprintf("Aggregation"))
plotCI(x=mean$Group.1, y=mean$x, uiw=sd$x, err="y", pch=20, slty=3, scol = "red", add=TRUE)
plotCI(x=meanG$Group.1, y=meanG$x, uiw=sdG$x, err="y", pch=20, slty=3, scol = "blue", add=TRUE)
lines(mean$Group.1, mean$x, col="red", lw=4)
lines(meanG$Group.1, meanG$x, col="blue", lw=4)
...
mean <- aggregate(d3$Up, by = list(d3$F), mean)
sd <- aggregate(d3$Up, by = list(d3$F), sd)
plot(NA, ylab="Up", xlim=c(0,150), ylim=c(0,1), xlab=sprintf("Focal"))
plotCI(x=mean$Group.1, y=mean$x, uiw=sd$x, err="y", pch=20, slty=3, scol = "red", add=TRUE)
lines(mean$Group.1, mean$x, col="red", lw=4)
legend("topleft", c("Focal G*") , fill=c("red"))

mean <- aggregate(d3$Down, by = list(d3$F), mean)
sd <- aggregate(d3$Down, by = list(d3$F), sd)
plot(NA, ylab="Down", xlim=c(0,150), ylim=c(0,1), xlab=sprintf("Focal"))
plotCI(x=mean$Group.1, y=mean$x, uiw=sd$x, err="y", pch=20, slty=3, scol = "red", add=TRUE)
lines(mean$Group.1, mean$x, col="red", lw=4)
legend("topleft", c("Focal G*") , fill=c("red"))
