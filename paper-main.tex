...
\begin{itemize}
\item hot spot analysis is important
\item similar to cluster analysis
\item is important to identify interesting areas 
\item typical problem is the correct detection method and parametrization
\item inital work of bruns and viliam 2017 show the instability and present simple metric and stable  method
\item here, we build upon that work and examine in more detail the impact of the different parameter with the standard $G^*$ and focal $G^*$, in particular the size of the study area (focal matrix), the detail of the resolution (zoom) and the size of the neighbourhood (weight matrix)
\item in particular, we compare the stability for all possible combinations and isolate the effect of single parameter by clever aggregation
\item We test on well-known taxi data set for easy comparison and so that in can be easily replicated
\end{itemize}
...
\item typical clustering apporaches -> phd symposium as examples
\end{itemize}
