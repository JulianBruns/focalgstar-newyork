...
To evaluate this dimension it is essential to 
know how big the study area needs to be. Based on these results we want to fix the focal size F
...
Higher values means that the found hot spots are more stable. 
The \SOHUP in figure ~\ref{fig:fUp} growth with an increase of the focal size.
We can see that an increase of the focal size leads to better results for \SOHUP. 
...
at focal size F of 17x17 there is no standard deviation. This is presented with the absent of the dotted line.
At focal size F 17x17 the standard deviation is missing, because the weight size W must be smaller than F. 
And for focal size F the values for W, which are caluclated, are only 7,11,15.
...
In figure ~\ref{fig:wUp} one can see that an increase of the weight size leads to better results for the \SOHUP. 
We compare $G^*$ wiht Focal $G^*$ . The Focal $G^*$ is not always above the $G^*$.
Therefore, Focal $G^*$ is not always better than $G^*$, which would be a perfect result we hoped for. It seems 
that $G^*$ has at 27 and 43 some small break downs. 
...
The same images as before are used, but the \SOHDOWN is calculated.
The high standard deviation in general indicates that it is possible 
to use better parametrisation for better results, which would be the aggregation size.
The break downs at 27 and 43 are also visible, but this time the standard deviation is increasing.
The \SOHDOWN and \SOHUP is mostly better than $G^*$ with fixed x-axis for the weight. 
That was the conclusion in \ref{TODO last paper}. 
...
The results can be seen in figure ~\ref{fig:SoHZoom}. 
...
\ref{fig:SoHZoom}. At aggregation level 5 $G^*$ is worse than Focal $G^*$, but at this point $G^*$ is extreme 
...
The result in Fig.~\ref{fig:Zoom} are consistent that $G^*$ is always better than Focal $G^*$, but suprisingly 
we found an outlier.
...
W is changed from 7 to 29 with a stepsize of 2, F and the aggregation level are fixed.
...
that an increase of the weight leads to better results. This slice plane is not consistent. Focal $G^*$ is even decreasing.
...
weights. Focal $G^*$ is mostly above 0.4.
...
Our results indicate that the parametrisation for $G^*$ and Focal $G^*$ besides what is defined as parent and child,
has a huge impact on the
stability of hotspots. Wrong parametrisation can lead to good results, the mean reaches for \SOHUP in the weight dimension 
around 0.6.
It can also lead to bad results, \SOHUP in the aggregation dimension reaches nearly zero. Therefore, we shown that it is important 
to have a metric
for the stability of hotspots. We also presented indication which parametrisation should be avoided and which parametrisations 
are promising.
...
We didn't evaluate the results on other months because we assume that they are similar, but it is interesting to find clusters 
for the different times in the day.
One could assume that at lunchtime there will be hotspots at restaurants and they are not consistent with the hotspots over one month.
