...
Neigbours are defined if they can be reached with a stepsize of 1 in x and y direction, independent of each other.
Which can be expressed in R with clumping, TODO which package and parametrisations.
...
\subsection{Blur}
We also made an example for the variation of the weight size.
...
\begin{displaymath}
\SOHUP( G^*(R_z, W_i, st, F), G^*(R_{z}, W_{i+2}, st, F) )
\end{displaymath}
\begin{displaymath}
\SOHDOWN( G^*(R_z, W_i, st, F), G^*(R_{z}, W_{i+2}, st, F) )
\end{displaymath}
\begin{displaymath}
z =3, i \in [1,6],dim(F)=41, G^* \in [Standard, Focal]
\end{displaymath}
\end{definition}
The blur results are plotted in Fig.~\ref{fig:Blur}. We show how the weight matrix is increasing
and how $G^*$ and Focal $G^*$ changes bases on W.
