...
plot_temp_zscore_overlay <- function(
  tempr,
  gstar,
  sigma_thresh = 5,
  hotspot_quantile_thresh = .01,
  temp_colors = interpolate_colortable(howmany = 100,
                                       "blue", "green", "yellow", "red"),
  signif_transparency = .3,
  signif_color = c("black", "black"))
{
  par(mar = c(4,5,4,2) + .1)
  plot(tempr, axes = FALSE, alpha = .8,
       col = temp_colors,
       legend = FALSE)

  # plot significant stuff
  rth <- gstar
  rth[abs(gstar) < sigma_thresh] <- NA
  plot(rth, axes = FALSE, alpha = signif_transparency, add = TRUE,
       col = interpolate_colortable(10, signif_color[1], signif_color[2]),
       legend = FALSE)

  # contours = hotspots based on quantiles
  hotspots <- quantile(gstar, na.rm = TRUE,
                       probs = c(hotspot_quantile_thresh,
                                 1 - hotspot_quantile_thresh))
  contour(rth, lwd = 4, add = TRUE, levels = hotspots, col = c("blue", "red"))

  # legend explaining the hotspots (top / bottom)
  e <- extent(gstar)
  legend(e@xmax, e@ymin,
         xjust = 1,
         yjust = 0,
...
plot_clumping_diff <- function(r, st, child_size, parent_size,
                               col_child = alpha("yellow", 1),
                               col_parent = alpha("black", .5),
                               base_raster_alpha = .2,
                               cropto = extent(r))
{
  is.wholenumber <-
  ...
  stopifnot(is.wholenumber(child_size))
  stopifnot(is.wholenumber(parent_size))
  stopifnot(parent_size > child_size)

  wgen <- function(wsize) weight_matrix_circular_fade(wsize, wsize/4)

  r_child <- filter_qthresh(GetisOrd(r, wgen(child_size), st))
  r_parent <- filter_qthresh(GetisOrd(r, wgen(parent_size), st))

  r_child_clump <- suppressWarnings(clump(r_child))
  r_parent_clump <- suppressWarnings(clump(r_parent))

  # cropping
  r_child_clump <- crop(r_child_clump, cropto)
  r_parent_clump <- crop(r_parent_clump, cropto)

  par(mar = c(0, 0, 0, 1))

  ctable <- interpolate_colortable(howmany = 100,
                                   "blue", "green", "yellow", "red")
  plot(crop(r, cropto), col = alpha(ctable, base_raster_alpha))
  plot(r_child_clump,
       legend = FALSE, col = col_child, add = TRUE, axes = FALSE)
  plot(r_parent_clump,
       legend = FALSE, col = col_parent, add = TRUE, axes = FALSE)

  legend( cropto@xmax, cropto@ymax,
         xjust = 1.5, yjust = 1,
         bg = rgb(1, 1, 1, 0.85),
         legend = c(sprintf("Child granularity (%d cells)", child_size),
                    sprintf("Parent granularity (%d cells)", parent_size)),
         fill = c(col_child, col_parent))
}
